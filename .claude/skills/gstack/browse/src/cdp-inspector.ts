  ...
  if (matchedData.matchedCSSRules) {
    for (const match of matchedData.matchedCSSRules) {
      ...
      if (rule.selectorList?.selectors) {
        ...
        selectorText =
          rule.selectorList.selectors[matchingIdx]?.text || rule.selectorList.text || '';
      }
...
export async function undoModification(page: Page, index?: number): Promise<void> {
  ...
  if (idx < 0 || idx >= modificationHistory.length) {
    throw new Error(
      `No modification at index ${idx}. History has ${modificationHistory.length} entries.`,
    );
  }
...
export function formatInspectorResult(
  ...
  const w = Math.round(
    result.boxModel.content.width + result.boxModel.padding.left + result.boxModel.padding.right,
  );
  const h = Math.round(
    result.boxModel.content.height + result.boxModel.padding.top + result.boxModel.padding.bottom,
  );
