...
async function sendMessage() {
  ...
  const result = await new Promise((resolve) => {
    chrome.runtime.sendMessage(
      { type: 'sidebar-command', message: msg, tabId: sidebarActiveTabId },
      resolve,
    );
  });
...
function addEntry(entry) {
  ...
  if (entry.url)
    document.getElementById('footer-url')?.textContent &&
      (document.getElementById('footer-url').textContent = new URL(entry.url).hostname);
...
function renderBoxModel(data) {
  ...
  if (!box) {
    inspectorBoxmodel.innerHTML = '<span class="inspector-no-data">No box model data</span>';
    return;
  }
...
if (chatCleanupBtn)
  chatCleanupBtn.addEventListener('click', () => runCleanup(chatCleanupBtn, inspectorCleanupBtn));
