...
const avgE2ECost =
  e2eRuns.length > 0 ? e2eRuns.reduce((s, r) => s + r.total_cost_usd, 0) / e2eRuns.length : 0;
const avgJudgeCost =
  judgeRuns.length > 0 ? judgeRuns.reduce((s, r) => s + r.total_cost_usd, 0) / judgeRuns.length : 0;

// Duration + turns from E2E runs
const avgE2EDuration =
  e2eRuns.length > 0
...
const avgE2ETurns = e2eTurns.length > 0 ? e2eTurns.reduce((a, b) => a + b, 0) / e2eTurns.length : 0;
...
console.log(
  `  Total runs:        ${results.length} (${e2eRuns.length} e2e, ${judgeRuns.length} llm-judge)`,
);
