...
describeIfSelected('Land-and-Deploy skill E2E', ['land-and-deploy-workflow'], () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(landDir, 'app.ts'),
      'export function hello() { return "deployed"; }\n',
    );
    run('git', ['add', '.']);
  ...
  afterAll(() => {
    try {
      fs.rmSync(landDir, { recursive: true, force: true });
    } catch {}
  });
...
Do NOT use AskUserQuestion. Do NOT run gh or fly commands.`,
      ...
      if (fs.existsSync(claudeMd)) {
        ...
        const hasFly =
          content.toLowerCase().includes('fly') || content.toLowerCase().includes('test-app');
...
describeIfSelected('Land-and-Deploy first-run E2E', ['land-and-deploy-first-run'], () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(firstRunDir, 'app.ts'),
      'export function hello() { return "world"; }\n',
    );
    fs.writeFileSync(
      path.join(firstRunDir, 'fly.toml'),
      'app = "first-run-app"\n\n[http_service]\n  internal_port = 3000\n',
    );
    run('git', ['add', '.']);
    run('git', ['commit', '-m', 'initial']);
    ...
    fs.writeFileSync(
      path.join(firstRunDir, 'app.ts'),
      'export function hello() { return "first deploy"; }\n',
    );
    run('git', ['add', '.']);
...
Just demonstrate the first-run dry-run output.`,
      ...
      const hasPlatform =
        reportContent.toLowerCase().includes('fly') ||
        reportContent.toLowerCase().includes('first-run-app');
...
Show what the readiness gate output would look like.`,
      ...
      const hasReviewMention =
        reportContent.toLowerCase().includes('review') ||
