...
describe('gstack-global-discover', () => {
  describe('normalizeRemoteUrl', () => {
    ...
    test('converts SSH without .git to HTTPS', () => {
      expect(normalizeRemoteUrl('git@github.com:user/repo')).toBe('https://github.com/user/repo');
    });
  ...
  describe('CLI', () => {
    ...
    test('--since 7d --format summary produces readable output', () => {
      const result = spawnSync('bun', ['run', scriptPath, '--since', '7d', '--format', 'summary'], {
        encoding: 'utf-8',
        timeout: 30000,
      });
      expect(result.status).toBe(0);
    ...
    test('--since 1h returns results (may be empty)', () => {
      const result = spawnSync('bun', ['run', scriptPath, '--since', '1h', '--format', 'json'], {
        encoding: 'utf-8',
        timeout: 30000,
      });
      expect(result.status).toBe(0);
  ...
  describe('discovery output structure', () => {
    test('repos have required fields', () => {
      const result = spawnSync('bun', ['run', scriptPath, '--since', '30d', '--format', 'json'], {
        encoding: 'utf-8',
        timeout: 30000,
      });
      expect(result.status).toBe(0);
