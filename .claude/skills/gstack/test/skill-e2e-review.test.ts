...
Write your findings to ${dir}/review-output.md`,
        ...
        const usedGitDiff = result.toolCalls.some((tc) => {
          ...
          const cmd =
            typeof tc.input === 'string' ? tc.input : tc.input?.command || JSON.stringify(tc.input);
...
Write the dashboard output to ${dashDir}/dashboard-output.md`,
      ...
      const allOutput = [result.output || '', ...result.toolCalls.map((tc) => tc.output || '')]
        .join('\n')
        .toLowerCase();
      ...
      const gateQuestions = result.toolCalls.filter(
        (tc) => tc.tool === 'mcp__conductor__AskUserQuestion' || tc.tool === 'AskUserQuestion',
      );
