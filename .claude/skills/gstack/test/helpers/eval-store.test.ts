...
describe('EvalCollector', () => {
  ...
  test('written JSON has correct schema fields', async () => {
    const collector = new EvalCollector('e2e', tmpDir);
    collector.addTest(makeEntry({ passed: true, cost_usd: 0.1, duration_ms: 2000 }));
    collector.addTest(
      makeEntry({ name: 'test-2', passed: false, cost_usd: 0.05, duration_ms: 1000 }),
    );
    const filepath = await collector.finalize();
  ...
  test('double finalize does not write twice', async () => {
    ...
    expect(
      fs.readdirSync(tmpDir).filter((f) => f.endsWith('.json') && !f.startsWith('_partial')),
    ).toHaveLength(1);
  });
...
describe('extractToolSummary', () => {
  ...
  test('handles events with no content array', () => {
    const transcript = [{ type: 'assistant', message: {} }, { type: 'assistant' }];
...
describe('findPreviousRun', () => {
  test('finds correct file — same branch preferred, most recent', () => {
    ...
    const files = [
      {
        name: '0.3.5-main-e2e-20260312-100000.json',
        data: makeResult({ branch: 'main', timestamp: '2026-03-12T10:00:00Z' }),
      },
      {
        name: '0.3.5-feature-e2e-20260313-100000.json',
        data: makeResult({ branch: 'feature', timestamp: '2026-03-13T10:00:00Z' }),
      },
      {
        name: '0.3.6-feature-e2e-20260314-100000.json',
        data: makeResult({ branch: 'feature', timestamp: '2026-03-14T10:00:00Z' }),
      },
    ];
  ...
  test('filters by tier', () => {
    fs.writeFileSync(
      ...
      JSON.stringify(
        makeResult({ tier: 'llm-judge', branch: 'main', timestamp: '2026-03-14T10:00:00Z' }),
      ),
    );
...
describe('compareEvalResults', () => {
  ...
  test('computes cost and duration deltas', () => {
    const before = makeResult({ total_cost_usd: 2.0, total_duration_ms: 60000 });
    const after = makeResult({ total_cost_usd: 1.5, total_duration_ms: 45000 });

    const result = compareEvalResults(before, after, 'a.json', 'b.json');
    expect(result.total_cost_delta).toBe(-0.5);
    ...
    const comparison: ComparisonResult = {
      ...
      deltas: [
        ...
        },
        {
          ...
          before: { passed: false, cost_usd: 1.0, detection_rate: 3, tool_summary: {} },
  ...
  test('includes commentary section', () => {
    const comparison: ComparisonResult = {
      ...
      deltas: [
        {
          name: 'test-a',
          before: { passed: true, cost_usd: 0.5, turns_used: 20, duration_ms: 120000 },
          after: { passed: true, cost_usd: 0.3, turns_used: 10, duration_ms: 60000 },
          status_change: 'unchanged',
        },
        {
          name: 'test-b',
          before: { passed: true, cost_usd: 0.1, turns_used: 5, duration_ms: 20000 },
          after: { passed: true, cost_usd: 0.1, turns_used: 5, duration_ms: 20000 },
          status_change: 'unchanged',
        },
        {
          name: 'test-c',
      ...
      total_cost_delta: -0.2,
      total_duration_delta: -60000,
      improved: 0,
      regressed: 0,
      unchanged: 3,
      tool_count_before: 30,
      tool_count_after: 20,
    };
...
describe('generateCommentary', () => {
  test('flags regressions prominently', () => {
    const c: ComparisonResult = {
      ...
      deltas: [
        {
          name: 'critical-test',
          before: { passed: true, cost_usd: 0.1 },
          after: { passed: false, cost_usd: 0.1 },
  ...
  test('notes improvements', () => {
    const c: ComparisonResult = {
      ...
      deltas: [
        {
          name: 'fixed-test',
          before: { passed: false, cost_usd: 0.1 },
          after: { passed: true, cost_usd: 0.1 },
          status_change: 'improved',
        },
      ],
      total_cost_delta: 0,
  ...
  test('reports detection rate changes', () => {
    const c: ComparisonResult = {
      ...
      deltas: [
        {
          name: 'detection-test',
          before: { passed: true, cost_usd: 0.5, detection_rate: 3 },
          after: { passed: true, cost_usd: 0.5, detection_rate: 5 },
          status_change: 'unchanged',
        },
      ],
      total_cost_delta: 0,
  ...
  test('produces overall summary for 3+ tests with no regressions', () => {
    const c: ComparisonResult = {
      ...
      deltas: [
        {
          name: 'a',
          before: { passed: true, cost_usd: 0.5, turns_used: 10, duration_ms: 60000 },
          after: { passed: true, cost_usd: 0.3, turns_used: 6, duration_ms: 40000 },
          status_change: 'unchanged',
        },
        {
          name: 'b',
          before: { passed: true, cost_usd: 0.2, turns_used: 5, duration_ms: 30000 },
          after: { passed: true, cost_usd: 0.15, turns_used: 4, duration_ms: 25000 },
          status_change: 'unchanged',
        },
        {
          name: 'c',
          before: { passed: true, cost_usd: 0.1, turns_used: 3, duration_ms: 20000 },
          after: { passed: true, cost_usd: 0.08, turns_used: 3, duration_ms: 18000 },
          status_change: 'unchanged',
        },
      ],
      total_cost_delta: -0.27,
      total_duration_delta: -27000,
      improved: 0,
      regressed: 0,
      unchanged: 3,
      tool_count_before: 0,
  ...
  test('returns empty for stable run with no significant changes', () => {
    const c: ComparisonResult = {
      ...
      deltas: [
        {
          name: 'a',
          before: { passed: true, cost_usd: 0.1, turns_used: 5, duration_ms: 20000 },
          after: { passed: true, cost_usd: 0.1, turns_used: 5, duration_ms: 21000 },
          status_change: 'unchanged',
        },
        {
          name: 'b',
      ...
      total_cost_delta: 0,
      total_duration_delta: 1000,
      improved: 0,
      regressed: 0,
      unchanged: 3,
      tool_count_before: 15,
      tool_count_after: 15,
    };
