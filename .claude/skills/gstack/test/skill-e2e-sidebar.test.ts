  ...
  testIfSelected(
    ...
    async () => {
      ...
      evalCollector?.addTest({
        name: 'sidebar-url-accuracy',
        suite: 'Sidebar URL accuracy E2E',
        tier: 'e2e',
        passed: true,
        duration_ms: 0,
        cost_usd: 0,
  ...
  testIfSelected(
    ...
    async () => {
      ...
      const resp = await api('/sidebar-command', {
        ...
        body: JSON.stringify({
          message:
            'Go to https://news.ycombinator.com. Find the top story. Click into its comments. Read the comments and find the most insightful one. Highlight that comment with a 4px solid orange outline.',
      ...
      if (!doneEntry || entries.length === 0) {
        ...
        console.log(
          ...
          agentProc?.exitCode,
          'signalCode:',
          agentProc?.signalCode,
          'killed:',
          agentProc?.killed,
        );
        const queueContent = fs.existsSync(queueFile)
          ? fs.readFileSync(queueFile, 'utf-8').slice(-500)
          : 'NO QUEUE';
      ...
      const allEntryText = entries
        ...
        .join(' ');
      const navigatedToHN =
        allEntryText.includes('ycombinator') ||
        allEntryText.includes('Hacker News') ||
        allEntryText.includes('news.ycombinator');
      if (!navigatedToHN) {
      ...
      const appliedStyle =
        allText.includes('outline') || allText.includes('orange') || allText.includes('style');

      evalCollector?.addTest({
        name: 'sidebar-css-interaction',
        suite: 'Sidebar CSS interaction E2E',
        tier: 'e2e',
