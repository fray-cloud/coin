...
describe('extractBrowseCommands', () => {
  test('extracts $B commands from bash code blocks', () => {
    const p = writeFixture(
      'basic.md',
      ['# Test', '```bash', '$B goto https://example.com', '$B snapshot -i', '```'].join('\n'),
    );
  ...
  test('skips non-bash code blocks', () => {
    const p = writeFixture(
      'skip.md',
      ['```json', '{"key": "$B goto bad"}', '```', '```bash', '$B text', '```'].join('\n'),
    );
    const cmds = extractBrowseCommands(p);
  ...
  test('returns empty array for code blocks with no $B invocations', () => {
    const p = writeFixture('no-b.md', ['```bash', 'echo "hello"', 'ls -la', '```'].join('\n'));
    const cmds = extractBrowseCommands(p);
    expect(cmds).toHaveLength(0);
  });
  ...
  test('handles multiple $B commands on one line', () => {
    const p = writeFixture(
      'multi.md',
      ['```bash', '$B click @e3       $B fill @e4 "value"     $B hover @e1', '```'].join('\n'),
    );
    const cmds = extractBrowseCommands(p);
  ...
  test('handles quoted arguments correctly', () => {
    const p = writeFixture(
      'quoted.md',
      ['```bash', '$B fill @e3 "test@example.com"', '$B js "document.title"', '```'].join('\n'),
    );
    const cmds = extractBrowseCommands(p);
  ...
  test('skips unlabeled code blocks', () => {
    const p = writeFixture('unlabeled.md', ['```', '$B snapshot -i', '```'].join('\n'));
    const cmds = extractBrowseCommands(p);
    expect(cmds).toHaveLength(0);
  });
});
...
describe('validateSkill', () => {
  ...
  test('invalid commands flagged in result', () => {
    const p = writeFixture(
      'invalid.md',
      ['```bash', '$B goto https://example.com', '$B explode', '$B halp', '```'].join('\n'),
    );
    const result = validateSkill(p);
  ...
  test('snapshot flags validated via parseSnapshotArgs', () => {
    const p = writeFixture('bad-snapshot.md', ['```bash', '$B snapshot --bogus', '```'].join('\n'));
    const result = validateSkill(p);
