...
function logCost(
  label: string,
  result: {
    costEstimate: { turnsUsed: number; estimatedTokens: number; estimatedCost: number };
    duration: number;
  },
) {
...
Remember: _SESSIONS=4, so ELI16 mode is active. The user is juggling multiple windows and may not remember what this conversation is about. Re-ground them.`,
        ...
        if (fs.existsSync(outputPath)) {
          ...
          expect(
            lower.includes('stripe') || lower.includes('checkout') || lower.includes('payment'),
          ).toBe(true);
...
The diff adds a new "returned" status to the Order model. Your job is to check if all consumers handle it.`,
    ...
    if (fs.existsSync(reviewPath)) {
      ...
      const mentionsEnum =
        review.toLowerCase().includes('enum') || review.toLowerCase().includes('status');
...
const anyOutcomeSelected =
  selectedTests === null || outcomeTestNames.some((t) => selectedTests!.includes(t));
...
describeIfSelected('Retro E2E', ['retro'], () => {
  ...
  beforeAll(() => {
    ...
    fs.copyFileSync(path.join(ROOT, 'retro', 'SKILL.md'), path.join(retroDir, 'retro', 'SKILL.md'));
  });
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    expect(
      statusLines.filter((l: string) => l.startsWith(' M') || l.startsWith('M ')),
    ).toHaveLength(0);
  }, 240_000);
});
...
- After editing, just commit the changes locally.`,
    ...
    const readmeUpdated =
      readme.includes('Feature C') || readme.includes('feature-c') || readme.includes('feature C');
...
IMPORTANT: The install directory is at ./.claude/skills/gstack — use that exact path.`,
      ...
      recordE2E('/gstack-upgrade happy path', 'gstack-upgrade E2E', result, {
        passed:
          versionAfter === '0.6.0' && ['success', 'error_max_turns'].includes(result.exitReason),
      });
...
Output the diagram directly.`,
    ...
    const hasTested =
      outputLower.includes('tested') || output.includes('✓') || output.includes('★');
...
Write the full output (including the GATE verdict) to ${codexDir}/codex-output.md`,
    ...
    if (fs.existsSync(outputPath)) {
      ...
      const hasCodexOutput =
        output.includes('CODEX') || output.includes('GATE') || output.includes('codex');
