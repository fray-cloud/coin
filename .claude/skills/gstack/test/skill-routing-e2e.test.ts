  ...
  testIfSelected(
    ...
    async () => {
      ...
      try {
        ...
        expect([expectedSkill], `Expected skill ${expectedSkill} but got ${actualSkill}`).toContain(
          actualSkill,
        );
      } finally {
    ...
    async () => {
      ...
      try {
        fs.writeFileSync(
          ...
          JSON.stringify({ name: 'waitlist-app', scripts: { dev: 'next dev' } }, null, 2),
        );
        fs.mkdirSync(path.join(tmpDir, 'src'), { recursive: true });
        fs.writeFileSync(
        ...
        expect(acceptable, `Expected skill ${expectedSkill} but got ${actualSkill}`).toContain(
          actualSkill,
        );
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
