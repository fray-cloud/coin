...
describeIfSelected('Review Army: Migration Safety', ['review-army-migration-safety'], () => {
  ...
  beforeAll(() => {
    ...
    const migrationContent = fs.readFileSync(
      path.join(ROOT, 'test', 'fixtures', 'review-army-migration.sql'),
      'utf-8',
    );
    fs.writeFileSync(
      path.join(dir, 'db', 'migrate', '20260330_drop_columns.sql'),
      migrationContent,
    );
    repo.run('git', ['add', '.']);
...
describeIfSelected('Review Army: N+1 Performance', ['review-army-perf-n-plus-one'], () => {
  ...
  beforeAll(() => {
    ...
    const n1Content = fs.readFileSync(
      path.join(ROOT, 'test', 'fixtures', 'review-army-n-plus-one.rb'),
      'utf-8',
    );
...
Write your completion audit to ${dir}/review-output.md`,
      ...
      if (fs.existsSync(outputPath)) {
        ...
        const mentionsEmail = content.includes('email') || content.includes('notification');
...
describeIfSelected('Review Army: Quality Score', ['review-army-quality-score'], () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(dir, 'user_controller.rb'),
...
Include the line: "PR Quality Score: X/10" where X is the computed score.`,
      ...
      if (fs.existsSync(outputPath)) {
        ...
        const hasScore =
          content.toLowerCase().includes('quality score') || content.match(/\d+\/10/);
...
describeIfSelected('Review Army: Consensus', ['review-army-consensus'], () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(dir, 'auth_controller.rb'),
      `class AuthController
  def login
