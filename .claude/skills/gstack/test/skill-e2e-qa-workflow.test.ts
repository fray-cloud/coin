...
Do NOT fix any bugs. Do NOT use AskUserQuestion — just pick vitest.`,
      ...
      const hasTestConfig =
        fs.existsSync(path.join(bsDir, 'vitest.config.ts')) ||
      ...
      console.log(
        `Test config: ${hasTestConfig}, Test file: ${hasTestFile}, TESTING.md: ${hasTestingMd}`,
      );

      try {
        fs.rmSync(bsDir, { recursive: true, force: true });
      } catch {}
    },
    120_000,
  );
