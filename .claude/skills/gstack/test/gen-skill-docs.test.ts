    ...
    for (const skill of ALL_SKILLS) {
      ...
      for (const block of bashBlocks) {
        ...
        for (const line of lines) {
          ...
          const isGlobCmd =
            /\b(?:ls|cat|rm|grep)\b/.test(trimmed) && /(?:\/\*[a-z.*]|\*\.[a-z])/.test(trimmed);
          if (isGlobCmd) {
...
describe('Codex generation (--host codex)', () => {
  ...
  test('--host agents alias produces same output as --host codex', () => {
    ...
    const agentsResult = Bun.spawnSync(
      ['bun', 'run', 'scripts/gen-skill-docs.ts', '--host', 'agents', '--dry-run'],
      {
        cwd: ROOT,
        stdout: 'pipe',
        stderr: 'pipe',
...
describe('Factory generation (--host factory)', () => {
  ...
  test('--host droid alias works', () => {
    const factoryResult = Bun.spawnSync(
      ['bun', 'run', 'scripts/gen-skill-docs.ts', '--host', 'factory', '--dry-run'],
      {
        cwd: ROOT,
        stdout: 'pipe',
        stderr: 'pipe',
    ...
    const droidResult = Bun.spawnSync(
      ['bun', 'run', 'scripts/gen-skill-docs.ts', '--host', 'droid', '--dry-run'],
      {
        cwd: ROOT,
        stdout: 'pipe',
        stderr: 'pipe',
