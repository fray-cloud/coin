...
describe('matchGlob', () => {
  ...
  test('* matches within a single segment', () => {
    expect(
      matchGlob('test/fixtures/review-eval-enum.rb', 'test/fixtures/review-eval-enum*.rb'),
    ).toBe(true);
    expect(
      matchGlob('test/fixtures/review-eval-enum-diff.rb', 'test/fixtures/review-eval-enum*.rb'),
    ).toBe(true);
    expect(
      matchGlob('test/fixtures/review-eval-vuln.rb', 'test/fixtures/review-eval-enum*.rb'),
    ).toBe(false);
  });
...
describe('selectTests', () => {
  ...
  test('multiple changed files union their selections', () => {
    const result = selectTests(['plan-ceo-review/SKILL.md', 'retro/SKILL.md.tmpl'], E2E_TOUCHFILES);
    expect(result.selected).toContain('plan-ceo-review');
    expect(result.selected).toContain('plan-ceo-review-selective');
...
describe('TOUCHFILES completeness', () => {
  ...
  test('every LLM-judge test has a TOUCHFILES entry', () => {
    const llmContent = fs.readFileSync(path.join(ROOT, 'test', 'skill-llm-eval.test.ts'), 'utf-8');
